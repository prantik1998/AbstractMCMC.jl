...
function mcmcsample(
    ...
    function swap_β(rng,model,samplers::Vector{<:AbstractSampler}, states, start::Integer,total_swap_moves,accepted_swap_moves,samples_per_replica)
        L = length(samplers) - 1
        for sampler_id in start:2:L
            ...
            if log(1-Random.rand(rng)) ≤ logα
                ...
                samples_per_replica[sampler_id+1] = save!!(samples_per_replica[sampler_id], sample, i, model, sampler, N; kwargs...)
                state = states[sampler_id+1] 
                sampler = samplers[sampler_id+1]
                sample, state = step(rng, model, sampler, state; kwargs...)
                callback === nothing || callback(rng, model, sampler, sample, state, i; kwargs...)
                states[sampler_id] = state
                samples_per_replica[sampler_id] = save!!(samples_per_replica[sampler_id+1], sample, i, model, sampler, N; kwargs...)
    ...
    @ifwithprogresslogger progress name=progressname begin
        ...
        for i in 2:N
            ...
            if i % swap_every == 0
                if i % (2*swap_every) == 0
                    swap_β(rng,model,samplers, states, 2,total_swap_moves,accepted_swap_moves,samples_per_replica) # swap even indices
                else
                    swap_β(rng,model,samplers, states, 1,total_swap_moves,accepted_swap_moves,samples_per_replica) # swap odd indices
                end
            end
            for (sampler_id, sampler) in enumerate(samplers)
